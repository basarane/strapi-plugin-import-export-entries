...
export const GenerateModal = ({ onClose }) => {
      ...
      models.forEach((model) => {
        for (const attr in model.attributes) {
          if (recurseTypes.includes(model.attributes[attr].type)) {
            ...
            if (component)
              model.attributes[attr].attributes = component.attributes;
            const relation = models.find((m) => m.uid === model.attributes[attr].target);
            if (relation)
              model.attributes[attr].attributes = relation.attributes;
          }
        }
      });
  ...
  if (data && data.models && options.collection) {
    ...
    if (model.kind === "singleType")
      availableTemplates = templates.filter(p => !p.isCollection);
  }
            ...
            <>
              <Grid gap={8}>
                <GridItem col={12}>
                  <Select
                    ...
                    onChange={handleSetOption('collection')}
                  >
              ...
              <Flex direction="column" alignItems="start" gap="16px">
                <Select
                  ...
                  onChange={handleSetOption('template')}
                >
